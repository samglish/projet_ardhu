...
<?php
    ...
    if (!empty($requete))
    {
        $req = "SELECT * FROM pdf_data WHERE file_des LIKE '%$requete%' and file_date LIKE 2024 and file_rep LIKE 'CF24'"; 
        $exec = $conn->query($req);                            
// exécuter la requête
...
<center>
       ...
       ';
$sql = "SELECT * from pdf_data  WHERE file_date LIKE 2024 and file_rep LIKE 'CF24' order by id desc";
$query = $dbh -> prepare($sql);
$query->execute();
